...
from diff2.pos_tagger import POSTagger
...
class TestCrossValidation(unittest.TestCase):
    ...
    def setUp(self):
        self.files = glob.glob('data/inauguration/*.txt')
    ...
    def test(self):
        for i in range(TestCrossValidation.FOLDS):
            ...
            validation_files = [fn for idx, fn in enumerate(self.files) if idx in validation_indexes]
            training_files = [fn for idx, fn in enumerate(self.files) if idx in training_indexes]
